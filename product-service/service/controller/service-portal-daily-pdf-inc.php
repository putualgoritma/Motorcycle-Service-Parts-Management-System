...
ORDER BY motorcycle_type.motorcycle_type_name ASC,service_order.service_order_id ASC");
...
if(isset($service_in_service_total_arr['total'][$category_code])){
	...
	$service_in_service_total_arr['total_price'][$category_code]+=$service_in_service_order_row['select_data'][$i]['price'];
	}else{
	$service_in_service_total_arr['total'][$category_code]=$service_in_service_order_row['select_data'][$i]['quantity'];
	$service_in_service_total_arr['total_price'][$category_code]=$service_in_service_order_row['select_data'][$i]['price'];
	}
...
$product_in_service_order_row=$global->db_qry_data("SELECT GROUP_CONCAT(service_order.service_order_id),category.category_name,category.category_code,
...
//qry product in service_order group by category
$product_in_product_order_row=$global->db_qry_data("SELECT GROUP_CONCAT(product_order.product_order_id),category.category_name,category.category_code,
SUM(product_orderdetails_total) AS price
FROM product_order
...
//mechanic
$staff_row=$global->db_qry_data("SELECT staff.*,position.position_name,position.position_code FROM staff INNER JOIN (SELECT position_name,position_code FROM position WHERE position.position_code = 'MK') position ON staff.position_code = position.position_code");
$mechanic_num=count($staff_row['select_data']);
//total part
$product_in_service_oli=0;
$product_in_service_total=0;
for($i=0;$i<count($product_in_service_order_row['select_data']);$i++){
if($product_in_service_order_row['select_data'][$i]['category_code']=="OLI"){
	$product_in_service_oli+=$product_in_service_order_row['select_data'][$i]['price'];
	}else{
	$product_in_service_total+=$product_in_service_order_row['select_data'][$i]['price'];
	}
}
//grand total
$grand_total_income=$servicetot_arr['TOTAL']+$product_in_service_total+$product_in_service_oli;
?>
