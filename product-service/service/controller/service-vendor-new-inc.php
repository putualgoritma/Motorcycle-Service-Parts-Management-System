<?
//default
$service_vendor_code_generation=$global->product_order->generator_service_vendor_code("pmn");
$service_order_queue_generation=$global->product_order->generator_service_vendor_queue();
//cancell
...
if($global->tbldata_exist("service_order","service_order_id","service_order_code='".$service_order_code."'") || $valid_date['date_monthnum']!=$date_nownum){
	...
	//insert
	if(!$global->product_order->insert_service_vendor($insert_arr,$service_order_total_hidden)){
	...
	$num_list=count($_POST['service_bcode_hidden']);
	foreach($_POST['service_bcode_hidden'] as $key => $service_bcode_hidden_val ) {
		...
		//echo $service_orderdetails_cost_addon."<br>";
