<?
if(isset($_REQUEST['service_order_monthly'])){
$service_order_monthly=$_REQUEST['service_order_monthly'];
} else if(isset($_POST['service_order_monthly'])){
$service_order_monthly=$_POST['service_order_monthly'];
}else{
$service_order_monthly=date("m/Y");
}
//qry service in service_order group by category
$service_in_service_order_row=$global->db_qry_data("SELECT service_order.service_order_id,motorcycle_type.motorcycle_type_code,motorcycle_type.motorcycle_type_name,category.category_code,motorcycle.motorcycle_type_code as motorcycle_type_code_def,motorcycle.motorcycle_code,
...
WHERE service_order.service_order_status='pmn' AND service_order.service_order_type='si' AND service_order_payregister LIKE '%".$service_order_monthly."'
GROUP BY category.category_code,motorcycle.motorcycle_type_code,service_order.service_order_id
ORDER BY motorcycle.motorcycle_type_code ASC,service_order.service_order_id ASC");
...
if($i==0 || $motorcycle_type_code_set!=$service_in_service_order_row['select_data'][$i]['motorcycle_type_code_def']){
	$motorcycle_type_code_set=$service_in_service_order_row['select_data'][$i]['motorcycle_type_code_def'];
	$ue=0;
	$tj=0;
	$j++;
	if($i==0){
...
$service_in_service_order_arr[$j]['motorcycle_type_name'] = $service_in_service_order_row['select_data'][$i]['motorcycle_type_name'] != '' ? $service_in_service_order_row['select_data'][$i]['motorcycle_type_name'] : $service_in_service_order_row['select_data'][$i]['motorcycle_type_code_def'];
...
LEFT JOIN product ON product_orderdetails.product_code = product.product_code
LEFT JOIN category ON product.category_code = category.category_code
WHERE product_order.product_order_status='pmn' AND product_orderdetails.product_orderdetails_type='si' AND product_order_register LIKE '%".$service_order_monthly."'
GROUP BY category.category_code
ORDER BY category.category_name ASC");
...
//qry service in service_order group by category
$service_in_service_order_staff_row=$global->db_qry_data("SELECT service_order.service_order_id,staff.staff_code,staff.staff_name,category.category_code,
SUM(service_orderdetails_quantity) AS quantity,
SUM(service_orderdetails_total) AS price
FROM service_order
...
//qry expense
$taxonomi_expense_row = $global->db_qry_data("SELECT GROUP_CONCAT(ledger.ledger_register),GROUP_CONCAT(ledger.ledger_code),GROUP_CONCAT(ledger.ledger_description),
taxonomi.taxonomi_code,taxonomi.taxonomi_name,
SUM(CASE WHEN ledgerdetails.ledgerdetails_type ='D' THEN ledgerdetails.ledgerdetails_amount ELSE 0 End) as ledgerdetails_amount_debit,
SUM(CASE WHEN ledgerdetails.ledgerdetails_type ='K' THEN ledgerdetails.ledgerdetails_amount ELSE 0 End) as ledgerdetails_amount_credit
FROM ledger
LEFT JOIN ledgerdetails ON ledger.ledger_id = ledgerdetails.ledger_id
LEFT JOIN taxonomi ON ledgerdetails.taxonomi_id = taxonomi.taxonomi_id
WHERE (ledger.ledger_register LIKE '%".$service_order_monthly."') AND ledger.ledger_status = 'pmn' AND taxonomi.taxonomi_type = 'Biaya' AND taxonomi.taxonomy_special_type != 'hpp' AND taxonomi.taxonomy_special_type != 'expense_opname' AND taxonomi.taxonomy_special_type != 'expense_trade' AND taxonomi.taxonomy_special_type != 'expense_opname' AND taxonomi.taxonomy_special_type != 'expense_grant'
GROUP BY taxonomi.taxonomi_id
ORDER BY ledger.ledger_registernum ASC, GROUP_CONCAT(ledger.ledger_id) ASC");
?>
