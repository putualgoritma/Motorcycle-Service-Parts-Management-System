...
//default
$product_order_sale_code_generation=$global->product_order->generator_product_order_sale_code("pmn");
...
if($global->tbldata_exist("product_order","product_order_id","product_order_code='".$product_order_code."'") || $valid_date['date_monthnum']!=$date_nownum){
	...
	$insert_err="";
	if(isset($_POST['Save'])){
	if(!$global->product_order->save_product_order_sale($insert_arr,$product_order_total_hidden)){
	...
	//print_r($_POST['product_scode_hidden']);
	...
	//end loop
	if($ht_order_id>0){
		$global->product_order->ht_realization($product_order_id,$ht_order_id);
		$global->product_order->ht_realization_update_status($ht_order_id);
		}
	}}
	//redirect
