...
Header("location: product-sale-payable.php?search=$search&sort=$sort&per_page=$per_page");
exit;
}
//paid
...
?>
<?
if(!isset($_SESSION['product_order_sale_drange1_sessi'])){
$_SESSION['product_order_sale_drange1_sessi']="01/".date('m')."/".date('Y');
}
if(!isset($_SESSION['product_order_sale_drange2_sessi'])){
$_SESSION['product_order_sale_drange2_sessi']=date('d')."/".date('m')."/".date('Y');
}
...
$users_code_value="";
$date_range1_value=$global->date_strtonum($_SESSION['product_order_sale_drange1_sessi']);
$date_range2_value=$global->date_strtonum($_SESSION['product_order_sale_drange2_sessi']);
...
$_SESSION['product_order_sale_drange1_sessi']=$_REQUEST['date_range1'];
}
...
$_SESSION['product_order_sale_drange2_sessi']=$_REQUEST['date_range2'];
}
//array set
...
'sort'=>$sort_value,
'join_match'=>$date_range_match." AND product_order.product_order_type='sr' AND product_order.product_order_status = 'pmn' AND product_order.product_order_pay_method = 'credit'".$users_code_match." GROUP BY product_order.product_order_id",
'join_id'=>"",
'join_tbl'=>array("payreceivable","users","product_orderdetails","payreceivable_details"),
'join_type'=>array("INNER JOIN","LEFT JOIN","LEFT JOIN"),
'join_key'=>array("payreceivable_id","users_code","product_order_id","payreceivable_id"),
'join_tbl_field'=>array("payreceivable_id,payreceivable_paid_status","users_code,users_name","product_order_id,(SUM(product_orderdetails_total)) AS product_orderdetails_price_total","payreceivable_id,(SUM(payreceivable_details_amount)) AS payreceivable_details_amount_total"),
...
$product_order_search_list=$global->tbl_searchjoin_list("product_order","product_order.*,case when product_order_type = 'si' then product_order_accountdebit else product_order_accountcredit end AS product_order_accountpay,case when product_order_type = 'pi' then product_orderdetails.product_orderdetails_price_total end AS product_order_amount_sale,case when product_order_type = 'si' then product_orderdetails.product_orderdetails_price_total end AS product_order_amount_sale,users.users_name,product_orderdetails.product_orderdetails_price_total+product_order_tax_val+product_order_cost AS product_orderdetails_price_total,(product_orderdetails.product_orderdetails_price_total-payreceivable_details.payreceivable_details_amount_total) AS amount_residu",$def_arr,$search_field_arr,1,$select_num,$qry_str_sort);
//next prev
if($per_page_value<=0){
$total_page=1;
$current_page=1;
