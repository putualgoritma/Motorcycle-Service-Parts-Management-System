...
Header("location: product-buy-payable.php?search=$search&sort=$sort&per_page=$per_page");
...
'join_match'=>$date_range_match." AND product_order.product_order_status = 'pmn' AND product_order.product_order_type = 'pi' AND product_order.product_order_pay_method = 'credit' ".$users_code_match." GROUP BY product_order.product_order_id",
...
$product_order_search_list=$global->tbl_searchjoin_list("product_order","product_order.*,case when product_order_type = 'si' then product_order_accountdebit else product_order_accountcredit end AS product_order_accountpay,case when product_order_type = 'pi' then product_orderdetails.product_orderdetails_price_total end AS product_order_amount_buy,case when product_order_type = 'si' then product_orderdetails.product_orderdetails_price_total end AS product_order_amount_sale,users.users_name,product_orderdetails.product_orderdetails_price_total+product_order_tax_val+product_order_cost AS product_orderdetails_price_total,(product_orderdetails.product_orderdetails_price_total-IFNULL(payreceivable_details.payreceivable_details_amount_total, '0')) AS amount_residu",$def_arr,$search_field_arr,1,$select_num,$qry_str_sort);
