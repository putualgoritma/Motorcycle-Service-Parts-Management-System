...
$product_order_buy_code_generation=$global->product_order->generator_product_order_buy_retur_code();
...
Header("location: product-buy-retur.php");
...
if($global->tbldata_exist("product_order","product_order_id","product_order_code='".$product_order_code."'") || $valid_date['date_monthnum']!=$date_nownum){
	$product_order_code=$global->product_order->generator_product_order_buy_retur_code($valid_date['date_monthnum']);
	}
	...
	$insert_err="";
	if(isset($_POST['Save'])){
	if(!$global->product_order->save_product_order_buy_retur($insert_arr,$product_order_total_hidden)){
		$global->product_order->error_message($global->product_order->err_msg);
		$insert_status=false;
		$insert_err=$global->product_order->err_msg;
		}}
	if(isset($_POST['Submit'])){
	if(!$global->product_order->insert_product_order_buy_retur($insert_arr,$product_order_total_hidden)){
		$global->product_order->error_message($global->product_order->err_msg);
		$insert_status=false;
		$insert_err=$global->product_order->err_msg;
		}}
	...
	Header("location: product-buy-retur.php?confirm=".$form_header_lang['add_new_button']);
