...
if (isset($_POST["id"]) && is_array($_POST["id"]) && count($_POST["id"]) > 0) 
	{ 
	foreach ($_POST["id"] as $warehouse_id) 
		{
		if(!$global->product_order->delete_warehouse($warehouse_id)){
...
$search_field_arr=array("warehouse_code","warehouse_description");
$def_arr=array(
...
$warehouse_search_list=$global->tbl_search_list("warehouse","*",$def_arr,$search_field_arr,1,$select_num,$qry_str_sort);
//next prev
if($per_page_value<=0){
$total_page=1;
$current_page=1;
