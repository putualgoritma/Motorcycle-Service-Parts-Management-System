...
if (isset($_POST["id"]) && is_array($_POST["id"]) && count($_POST["id"]) > 0) 
	{ 
	foreach ($_POST["id"] as $categorysub_id) 
		{
		if(!$global->product_order->delete_categorysub($categorysub_id)){
...
$year_value="%";
$month_value="%";
$sort_value="categorysub_id DESC";
$pageset_value=0;
$per_page_value=0;
...
'sort'=>$sort_value,
'join_match'=>" AND categorysub_type='1'",
'join_id'=>""
);
$categorysub_search_list=$global->tbl_search_list("categorysub","*",$def_arr,$search_field_arr,1,$select_num,$qry_str_sort);
//next prev
if($per_page_value<=0){
$total_page=1;
$current_page=1;
