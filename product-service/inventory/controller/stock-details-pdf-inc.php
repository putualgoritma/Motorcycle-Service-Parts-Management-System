...
$qry = "SELECT warehouse_stock.warehouse_stock_register,warehouse_stock.warehouse_stock_register,warehouse_stock_time,warehouse_stock.warehouse_stock_time,warehouse_stock.warehouse_stock_code,warehouse_stock_details.product_code,product.product_name,
product.unit_code,product.product_bprice,contact.contact_name,
(CASE WHEN (warehouse_stock.warehouse_stock_type='in' OR warehouse_stock.warehouse_stock_type='opname') THEN warehouse_stock_details.warehouse_stock_details_quantity ELSE 0 END) amount_in,
(CASE WHEN warehouse_stock.warehouse_stock_type='out' THEN warehouse_stock_details.warehouse_stock_details_quantity ELSE 0 END) amount_out,
(CASE WHEN (warehouse_stock.warehouse_stock_type='trs' AND warehouse_stock.warehouse_stock_category='transfer_in') THEN warehouse_stock_details.warehouse_stock_details_quantity ELSE 0 END) amount_trs_in,
(CASE WHEN (warehouse_stock.warehouse_stock_type='trs' AND warehouse_stock.warehouse_stock_category='transfer_out') THEN warehouse_stock_details.warehouse_stock_details_quantity ELSE 0 END) amount_trs_out
...
LEFT JOIN contact ON warehouse_stock.contact_code = contact.contact_code
...
ORDER BY warehouse_stock.warehouse_stock_registernum ASC,warehouse_stock_details.warehouse_stock_details_id ASC";
$warehouse_stock_row=$global->db_qry_data($qry);
...
SUM(CASE WHEN (warehouse_stock.warehouse_stock_type='trs' AND warehouse_stock.warehouse_stock_category='transfer_out') THEN warehouse_stock_details.warehouse_stock_details_quantity ELSE 0 END) amount_trs_out
		...
		GROUP BY warehouse_stock_details.product_code,warehouse_stock_details.warehouse_stock_details_id
		ORDER BY GROUP_CONCAT(warehouse_stock.warehouse_stock_registernum) ASC,warehouse_stock_details.warehouse_stock_details_id ASC";
