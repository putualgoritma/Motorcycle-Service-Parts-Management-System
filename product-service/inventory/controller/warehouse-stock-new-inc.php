<?
//default
$warehouse_stock_code_generation=$global->product_order->generator_warehouse_stock_edit_code();
//cancell
...
$warehouse_stock_code=$global->product_order->generator_warehouse_stock_edit_code();
if($_POST['warehouse_stock_type']=="out"){
	$warehouse_stock_category="trs_out";
	$warehouse_stock_code=$global->product_order->generator_warehouse_stock_edit_code("out");
	}
//validate form
...
if($global->tbldata_exist("warehouse_stock","warehouse_stock_id","warehouse_stock_code='".$warehouse_stock_code."'") || $valid_date['date_monthnum']!=$date_nownum){
	$warehouse_stock_code=$global->product_order->generator_warehouse_stock_edit_code("in",$valid_date['date_monthnum']);
	if($_POST['warehouse_stock_type']=="out"){
	...
	'warehouse_code'=>	$warehouse_code,
	'warehouse_stock_type'=>	$_POST['warehouse_stock_type'],
	...
	if($warehouse_stock_category=="trs_in"){
		$set_rekening=array($stock_trade_acc,"D",$adjust_amount,$income_grant_acc,"K",$adjust_amount);
		}else{
	...
	//redirect
	Header("location: warehouse-stock.php?confirm=".$form_header_lang['add_new_button']);
	exit;
}
?>
