            ...
            <aside class="right-side">                
                ...
                <section class="content-header">
                    <h1><? echo $global->product_order->product_order_lang['form_header_product_order_lang']['warehouse_bag']; ?></h1>
                    <ol class="breadcrumb">
                        <li><a href="<? echo $path; ?>index.php"><i class="fa fa-home"></i> Home</a></li>
                        <li><a href="<? echo $path; ?>index-sub.php?module_code=<? echo $parent_active; ?>"> Stock/Persediaan</a></li>
                        <li class="active"><? echo $global->product_order->product_order_lang['form_header_product_order_lang']['warehouse_bag']; ?></li>
                    </ol>
                </section>
                <!-- Main content -->
                    ...
                    <div class="row">
                        <div class="col-xs-12">
                            <div class="box box-danger">
                                ...
                                <div class="col-md-12">
                                <form action="warehouse-bag.php" method="post" enctype="multipart/form-data" name="formsearch" id="formsearch">
                                <input name="search" type="text" class="selectbox firstin" id="search" value="<? echo"$search_value"; ?>"size="20" />
                                <div class="page">From: </div>
                                <input class="selectbox" type="text" id="datepicker01" name="date_range1" value="<? echo $_SESSION['warehouse_bag_drange1_sessi']; ?>" />
                                <div class="page">to</div>
                                <input class="selectbox" type="text" id="datepicker02" name="date_range2" value="<? echo $_SESSION['warehouse_bag_drange2_sessi']; ?>" />
                      <select name="warehouse_stock_status" class="textbox selectbox" id="warehouse_stock_status">
                      <option value="" <? if($warehouse_stock_status_value==""){?>selected<? }?>>Semua Status</option>
                      <option value="tmp" <? if($warehouse_stock_status_value=="tmp"){?>selected<? }?>>Pending</option>
                      <option value="pmn" <? if($warehouse_stock_status_value=="pmn"){?>selected<? }?>>Diproses</option>
                      </select>
...
<div class="clear">&nbsp;</div>
                                    <table width="100%" class="table table-bordered table-hover table-fixed gridView" id="example1">
                                        <thead>
                                            <tr>
                                                ...
                                                <td align="center" width="2%">&nbsp;</td>
                                                <td align="center" width="15%"><strong><a href="warehouse-bag.php?search=<? echo $search_value; ?>&amp;sort=warehouse_stock.warehouse_stock_registernum <? if($sort_value=="warehouse_stock.warehouse_stock_registernum ASC"){?>DESC<? }else{?>ASC<? }?>" class="link_table_title"><? if($sort_value=="warehouse_stock.warehouse_stock_registernum ASC"){?><i class="fa fa-sort-alpha-asc"></i><? }else{?><i class="fa fa-sort-alpha-desc"></i><? }?>&nbsp;&nbsp;<? echo $global->product_order->product_order_lang['form_label_product_order_lang']['warehouse_stock_register']; ?></a></strong></td>
                                                <td width="11%" align="center"><strong><a href="warehouse-bag.php?search=<? echo $search_value; ?>&amp;sort=warehouse_stock.warehouse_stock_status <? if($sort_value=="warehouse_stock.warehouse_stock_status ASC"){?>DESC<? }else{?>ASC<? }?>" class="link_table_title">
                                                  <? if($sort_value=="warehouse_stock.warehouse_stock_status ASC"){?>
                                                  <i class="fa fa-sort-alpha-asc"></i>
                                                  <? }else{?>
                                                  <i class="fa fa-sort-alpha-desc"></i>
                                                  <? }?>
                                                  &nbsp;&nbsp;<? echo $global->product_order->product_order_lang['form_label_product_order_lang']['warehouse_stock_status']; ?></a></strong></td>
                                                <td align="center" width="11%"><strong><a href="warehouse-bag.php?search=<? echo $search_value; ?>&amp;sort=warehouse_stock.warehouse_stock_code <? if($sort_value=="warehouse_stock.warehouse_stock_code ASC"){?>DESC<? }else{?>ASC<? }?>" class="link_table_title"><? if($sort_value=="warehouse_stock.warehouse_stock_code ASC"){?><i class="fa fa-sort-alpha-asc"></i><? }else{?><i class="fa fa-sort-alpha-desc"></i><? }?>&nbsp;&nbsp;<? echo $global->product_order->product_order_lang['form_label_product_order_lang']['warehouse_stock_code']; ?></a></strong></td>
                                                <td align="center" width="17%"><strong><a href="warehouse-bag.php?search=<? echo $search_value; ?>&amp;sort=warehouse.warehouse_name <? if($sort_value=="warehouse.warehouse_name ASC"){?>DESC<? }else{?>ASC<? }?>" class="link_table_title"><? if($sort_value=="warehouse.warehouse_name ASC"){?><i class="fa fa-sort-alpha-asc"></i><? }else{?><i class="fa fa-sort-alpha-desc"></i><? }?>&nbsp;&nbsp;<? echo $global->product_order->product_order_lang['form_label_product_order_lang']['warehouse_name']; ?></a></strong></td>
                                                <td align="center" width="17%"><strong><a href="warehouse-bag.php?search=<? echo $search_value; ?>&amp;sort=warehouse_stock.warehouse_stock_category <? if($sort_value=="warehouse_stock.warehouse_stock_category ASC"){?>DESC<? }else{?>ASC<? }?>" class="link_table_title"><? if($sort_value=="warehouse_stock.warehouse_stock_category ASC"){?><i class="fa fa-sort-alpha-asc"></i><? }else{?><i class="fa fa-sort-alpha-desc"></i><? }?>&nbsp;&nbsp;<? echo $global->product_order->product_order_lang['form_label_product_order_lang']['warehouse_stock_category']; ?></a></strong></td>
				...
				$view_link="warehouse_bag_edit_modal_off";
				if($ulm_arr[$parent_active][$page_active]['users_level_module_access_edit']==1){
				$view_link="warehouse_bag_edit_modal";
				}
			?>
                                        <tr>
                                            ...
                                            <td bgcolor="<? echo"$bg_color"; ?>"><a href="javascript:;" class="link_table <? echo $view_link; ?>"><? echo $global->get_selectlist($form_selectlist_lang['warehouse_stock_status'],$warehouse_stock_search_list[$i]['warehouse_stock_status']); ?></a></td>
                                            ...
                                            <td class="company_stock_process_hidden td_hide"><a href="#" class="link_table"><? echo $company['company_stock_process']; ?></a></td>
                                            <td class="warehouse_stock_status_hidden td_hide"><a href="#" class="link_table"><? echo $warehouse_stock_search_list[$i]['warehouse_stock_status']; ?></a></td>
                                          </tr>
										  <?
				$inc++;
				}
                  ...
                  <table width="360" border="0" align="center" cellpadding="0" cellspacing="0" id="nexprev">
                      ...
                      <tr>
                        <td align="right"><? if($current_page>1) { ?>
                            <a href="warehouse-bag.php?search=<? echo"$search_value"; ?>&amp;year=<? echo"$year_value"; ?>&amp;month=<? echo"$month_value"; ?>&amp;date=<? echo $date_value; ?>&amp;per_page=<? echo $per_page_value; ?>&amp;project_id=<? echo $project_id_value; ?>&amp;sort=<? echo"$sort_value"; ?>&amp;pageset=0"><i class="fa fa-angle-double-left"></i></a> <a href="warehouse-bag.php?search=<? echo"$search_value"; ?>&amp;year=<? echo"$year_value"; ?>&amp;month=<? echo"$month_value"; ?>&amp;date=<? echo $date_value; ?>&amp;per_page=<? echo $per_page_value; ?>&amp;project_id=<? echo $project_id_value; ?>&amp;sort=<? echo"$sort_value"; ?>&amp;pageset=<? echo"$pageset_prev"; ?>"><i class="fa fa-angle-left"></i></a>
                            <? }?></td>
                        ...
                        <td><? if($current_page<$total_page) { ?>
                            <a href="warehouse-bag.php?search=<? echo"$search_value"; ?>&amp;year=<? echo"$year_value"; ?>&amp;month=<? echo"$month_value"; ?>&amp;date=<? echo $date_value; ?>&amp;per_page=<? echo $per_page_value; ?>&amp;project_id=<? echo $project_id_value; ?>&amp;sort=<? echo"$sort_value"; ?>&amp;pageset=<? echo"$pageset_next"; ?>"><i class="fa fa-angle-right"></i></a> <a href="warehouse-bag.php?search=<? echo"$search_value"; ?>&amp;year=<? echo"$year_value"; ?>&amp;month=<? echo"$month_value"; ?>&amp;date=<? echo $date_value; ?>&amp;per_page=<? echo $per_page_value; ?>&amp;project_id=<? echo $project_id_value; ?>&amp;sort=<? echo"$sort_value"; ?>&amp;pageset=<? echo"$pageset_last"; ?>"><i class="fa fa-angle-double-right"></i></a>
                            <? }?></td>
                      </tr>
                  </table>
                                  </form>
...
<div class="box-header text-center">
    ...
    <button class="btn btn-primary" id="btn_edit" onclick="location.href=''" type="button"><i class="fa fa-edit"></i> VIEW</button>&nbsp;&nbsp;&nbsp;<button class="btn btn-info" id="btn_process" onclick="location.href=''" type="button"><i class="fa fa-play-circle"></i><span class="btn_process"> PROSES</span></button>&nbsp;&nbsp;&nbsp;<button class="btn btn-warning" id="btn_pdf" onclick="location.href=''" type="button"><i class="fa fa-file-pdf-o"></i> PDF</button>
<div>&nbsp;</div>
     </div><!-- /.box-header -->
