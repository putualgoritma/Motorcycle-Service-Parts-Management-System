...
//salary_daily_target
function update_salary_daily_target($salary_daily_target_arr,$salary_daily_target_id)
	{
	$result = true;
	//validate
...
//salary_target
function update_salary_target($salary_target_arr,$salary_target_id)
	{
	$result = true;
	//validate
...
//absence_penalty
function update_absence_penalty($absence_penalty_arr,$absence_penalty_id)
	{
	$result = true;
	//validate
...
//company
function update_company($company_arr,$company_id)
	{
	$result = true;
	//validate
	//update company
...
//salary_slip
function delete_salary_slip($salary_slip_id)
	{
	$result = true;
	//validate
...
function pay_active_cashbon($cashbon_amount,$staff_code,$date_registernum)
	{
	//get cashbon residu
...
function pay_active_payable($payable_amount,$staff_code,$date_registernum)
	{
	//get payable residu
...
function create_salary_slip($salary_slip_arr)
	{
	$result = true;
	//validate
	...
	if($result){
		...
		$set_rekening=array($accountdebit,"D",$salary_slip_arr['salary_total'],$accountcredit,"K",$salary_slip_arr['salary_total']);
...
function update_salary_slip($salary_slip_arr,$salary_slip_id)
	{
	$result = true;
	//validate
	//if not redundant code
...
function generate_absence($month_year)
	{
...
function create_absence($absence_arr)
	{
	$result = true;
	//validate
	//if not redundant code
...
function update_absence($absence_arr,$absence_id)
	{
	$result = true;
	//validate
	//if not redundant code
...
//get absence
function get_absence($staff_code,$salary_slip_monthstrip)
	{
	$start_date=$salary_slip_monthstrip."-01";
	$end_date=$salary_slip_monthstrip."-31";
	//output
	$get_absence_arr=array();
	//get absence_penalty
...
//get service fee
function get_service_fee($staff_code,$salary_slip_monthnum)
	{
...
(CASE 
WHEN service.service_commission_type='percent' THEN (service_orderdetails_price*(service.service_commission_percent/100))
WHEN service.service_commission_type='nominal' THEN service.service_commission_nominal
ELSE 'no' 
...
WHERE staff.staff_code ='".$staff_code."' AND (service_order.service_order_registernum BETWEEN '".$start_date."' AND '".$end_date."') AND service_order.service_order_status='pmn' 
...
//get product fee
function get_product_fee($staff_code,$salary_slip_monthnum)
	{
	$start_date=intval($salary_slip_monthnum."01");
	$end_date=intval($salary_slip_monthnum."31");
...
(CASE 
WHEN product.product_commission_type='percent' THEN (product_orderdetails_price*(product.product_commission_percent/100))
WHEN product.product_commission_type='nominal' THEN product.product_commission_nominal
ELSE 'no' 
...
//get insentif
function get_daily_insentif($staff_code,$salary_slip_monthnum)
	{
	$start_date=intval($salary_slip_monthnum."01");
	$end_date=intval($salary_slip_monthnum."31");
	$salary_daily_target_amount=0;
	$get_insentif_arr=array();
	$pit_arr=array();
	$num_work=0;
...
//get insentif
function get_insentif($staff_code,$salary_slip_monthnum)
	{
	$start_date=intval($salary_slip_monthnum."01");
	$end_date=intval($salary_slip_monthnum."31");
	//output
	...
	for($i=0;$i<count($insentif_row['select_data']);$i++){
		...
		//product & service fee
		$get_service_fee=$this->get_service_fee($staff_code,$salary_slip_monthnum);
		$get_product_fee=$this->get_product_fee($staff_code,$salary_slip_monthnum);
		...
		//daily insentif
		$salary_daily_target_arr=$this->get_daily_insentif($staff_code,$salary_slip_monthnum);
...
//get service fee
function get_service_fee_nonpit($staff_code,$salary_slip_monthnum)
	{
	$start_date=intval($salary_slip_monthnum."01");
	$end_date=intval($salary_slip_monthnum."31");
	$service_fee=0;
	...
	$service_fee_row=$this->db_qry_data("SELECT service_order.service_order_code,service_order.service_order_register,
staff.staff_name,service.service_code,
SUM(service_orderdetails.service_orderdetails_total) AS amount_service,
(CASE 
WHEN service.service_commission_type='percent' THEN (service_orderdetails_price*(service.service_commission_percent/100))
...
ORDER BY service_order.service_order_code ASC");
	for($i=0;$i<count($service_fee_row['select_data']);$i++){
		...
		$service_fee+=((($service_amount*($company_row['service_fee_percent']/100))+$service_fee_amount)*0.2)/$num_staff_nonpit;
		}}
	return $service_fee;
	}
	
//get product fee
function get_product_fee_nonpit($staff_code,$salary_slip_monthnum)
	{
	$start_date=intval($salary_slip_monthnum."01");
	$end_date=intval($salary_slip_monthnum."31");
	$product_fee=0;
	$company_row=$this->db_row("company","*","");
	$product_fee_arr=array();
...
ORDER BY service_order.service_order_code ASC");
	for($i=0;$i<count($product_fee_row['select_data']);$i++){
		$absence_date=$this->date_stridtonum($product_fee_row['select_data'][$i]['service_order_register']);
		if($this->tbldata_exist("absence","absence_id","absence_date='".$absence_date."' AND staff_code = '".$staff_code."' AND absence_status='work'")){
		$product_fee_amount=$product_fee_row['select_data'][$i]['product_fee_amount'];
		$product_amount=$product_fee_row['select_data'][$i]['amount_product'];
		$absence_row=$this->db_row_join("absence,staff","count(absence.absence_id) as num_work","absence.absence_status='work' AND staff.position_code!='MK' AND absence.absence_date='".$absence_date."' AND absence.staff_code=staff.staff_code");
		$num_staff_nonpit=$absence_row['num_work'];
		$product_fee_arr[]['service_order_register']=$product_fee_row['select_data'][$i]['service_order_register'];
		$product_fee_arr[]['product_fee_amount']=$product_fee_amount;
		$product_fee_arr[]['product_amount']=$product_amount;
		$product_fee_arr[]['num_staff_nonpit']=$num_staff_nonpit;
		$product_fee_arr[]['product_fee']=((($product_amount*($company_row['product_fee_percent']/100))+$product_fee_amount)*0.2)/$num_staff_nonpit;
		$product_fee+=((($product_amount*($company_row['product_fee_percent']/100))+$product_fee_amount)*0.2)/$num_staff_nonpit;
		}}
	return $product_fee;
	}

//get insentif
function get_daily_insentif_nonpit($staff_code,$salary_slip_monthnum)
	{
	$start_date=intval($salary_slip_monthnum."01");
	$end_date=intval($salary_slip_monthnum."31");
	$salary_daily_target_amount=0;
...
//get insentif
function get_insentif_nonpit($staff_code,$salary_slip_monthnum)
	{
	$start_date=intval($salary_slip_monthnum."01");
	$end_date=intval($salary_slip_monthnum."31");
	//output
	...
	for($i=0;$i<count($insentif_row['select_data']);$i++){
		...
		$service_fee=$this->get_service_fee_nonpit($staff_code,$salary_slip_monthnum);
		$product_fee=$this->get_product_fee_nonpit($staff_code,$salary_slip_monthnum);
		//daily insentif
...
//get insentif
function get_efective_work($salary_slip_monthnum)
	{
	$start_date=intval($salary_slip_monthnum."01");
	$end_date=intval($salary_slip_monthnum."31");
	$num_work=0;
...
function generator_salary_slip_code($salary_slip_monthnum="")
	{
