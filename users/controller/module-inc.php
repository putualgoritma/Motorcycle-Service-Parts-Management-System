...
if (isset($_POST["id"]) && is_array($_POST["id"]) && count($_POST["id"]) > 0) 
	{ 
	foreach ($_POST["id"] as $module_id) 
		{
		if(!$global->users->delete_module($module_id)){
			$global->users->error_message($global->users->err_msg);
			}
		}
	}	
Header("location: module.php?search=$search&sort=$sort&pageset=$pageset&per_page=$per_page");
exit;
}
?>
<?
...
$year_value="%";
$month_value="%";
$sort_value="module_rank ASC,module_id ASC";
$pageset_value=0;
$per_page_value=0;
...
$search_field_arr=array("module_code","module_name");
$def_arr=array(
...
$module_search_list=$global->tbl_search_list("module","*",$def_arr,$search_field_arr,1,$select_num,$qry_str_sort);
//next prev
if($per_page_value<=0){
$total_page=1;
$current_page=1;
