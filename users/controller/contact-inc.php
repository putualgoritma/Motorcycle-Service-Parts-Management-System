...
if (isset($_POST["id"]) && is_array($_POST["id"]) && count($_POST["id"]) > 0) 
	{ 
	foreach ($_POST["id"] as $contact_id) 
		{
		if(!$global->users->delete_contact($contact_id)){
			$global->users->error_message($global->users->err_msg);
			}
		}
	}	
Header("location: contact.php?search=$search&sort=$sort&pageset=$pageset&per_page=$per_page");
exit;
}
?>
<?
...
'sort'=>$sort_value,
'join_match'=>" AND contact_username!=''",
'join_id'=>""
);
