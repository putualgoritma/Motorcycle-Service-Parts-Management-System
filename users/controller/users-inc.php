...
if (isset($_POST["id"]) && is_array($_POST["id"]) && count($_POST["id"]) > 0) 
	...
	foreach ($_POST["id"] as $users_id) 
		{
		$global->users->delete_users($users_id);
		}
	}	
Header("location: supplier.php?search=$search&year=$year&month=$month&sort=$sort&pageset=$pageset&per_page=$per_page");
exit;
}
?>
<?
...
'sort'=>$sort_value,
'join_match'=>" AND users_type !='member' AND users_type !='candidate' AND users_type !='fictive'",
'join_id'=>""
);
...
$inc=1+$pageset_value;
//echo $qry_str_sort;
$users_code_generation=$global->users->generator_kontak_code();
?>