...
if (isset($_POST["id"]) && is_array($_POST["id"]) && count($_POST["id"]) > 0) 
	{ 
	foreach ($_POST["id"] as $bank_id) 
		{
		if(!$global->users->delete_bank($bank_id)){
			$global->users->error_message($global->users->err_msg);
			}
		}
	}	
Header("location: bank.php?search=$search&sort=$sort&pageset=$pageset&per_page=$per_page");
exit;
}
?>
<?
