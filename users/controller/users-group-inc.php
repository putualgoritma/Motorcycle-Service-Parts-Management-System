...
if (isset($_POST["id"]) && is_array($_POST["id"]) && count($_POST["id"]) > 0) 
	{ 
	foreach ($_POST["id"] as $users_group_id) 
		{
		if(!$global->users->delete_users_group($users_group_id)){
			$global->users->error_message($global->users->err_msg);
			}
		}
	}	
Header("location: users-group.php?search=$search&sort=$sort&pageset=$pageset&per_page=$per_page");
exit;
}
?>
<?
...
$year_value="%";
$month_value="%";
$sort_value="users_group_id DESC";
$pageset_value=0;
$per_page_value=0;
