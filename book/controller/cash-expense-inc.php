...
Header("location: cash-expense.php");
exit;
}
?>
<?
...
'sort'=>$sort_value,
'join_match'=>$date_range_match." AND ledger.ledger_status = 'pmn' AND taxonomi.taxonomi_type = 'Biaya'".$ledger_id_qry,
'join_id'=>"",
...
$ledger_search_list=$global->tbl_searchjoin_multi_list("ledger","ledger.ledger_id,ledger.ledger_register,ledger.ledger_code,ledger.ledger_description,(CASE WHEN ledgerdetails.ledgerdetails_type ='D' THEN ledgerdetails.ledgerdetails_amount ELSE 0 End) as ledgerdetails_amount_debit,(CASE WHEN ledgerdetails.ledgerdetails_type ='K' THEN ledgerdetails.ledgerdetails_amount ELSE 0 End) as ledgerdetails_amount_credit,taxonomi.taxonomi_code,taxonomi.taxonomi_name",$def_arr,$search_field_arr,1,$select_num,$qry_str_sort);
//next prev
if($per_page_value<=0){
$total_page=1;
$current_page=1;
