<?
//default
$payreceivable_code_generation=$global->payreceivable->generator_receivable("",1);
$id_set_value="";
$amount_set_value=0;
//date
...
$payreceivable_row=$global->payreceivable->db_row("payreceivable","staff_code,payreceivable_accountdebit","payreceivable_id='".$_REQUEST['payreceivable_id']."'");
$payreceivable_accountcredit=$payreceivable_row['payreceivable_accountdebit'];
$staff_row=$global->payreceivable->db_row("staff","staff_code,staff_name","staff_code='".$payreceivable_row['staff_code']."'");
$staff_code=$staff_row['staff_code']." - ".$staff_row['staff_name'];
}
//echo $payreceivable_accountcredit;

//cancell
if(isset($_POST['Submitcancell']))
{
Header("location: receivable-min-new.php");
exit;
}
...
foreach($_POST['payreceivable_accountdebit_hidden'] as $key => $payreceivable_accountdebit_hidden_val ) {
	$payreceivable_code_arr=explode(" - ",$payreceivable_accountdebit_hidden_val);
	$add_str="";
	if($iset>0){
		$add_str=",";
		}
...
if(!$valid_date['is_valid']){
	...
	if(!$global->payreceivable->create_receivable($create_arr,0)){
	...
	//redirect
	Header("location: ../confirm.php?redirect=payreceivable/receivable-staff.php&confirm=".$form_header_lang['add_new_button']);
	exit;
}
?>
