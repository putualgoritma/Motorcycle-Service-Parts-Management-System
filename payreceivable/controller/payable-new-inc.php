<?
//default
$payreceivable_code_generation=$global->payreceivable->generator_payable();

//cancell
if(isset($_POST['Submitcancell']))
{
Header("location: payable-new.php");
exit;
}
...
if(!$valid_date['is_valid']){
	...
	$set_rekening=array($payreceivable_accountcredit,"D",$payreceivable_amount);
	foreach($_POST['payreceivable_accountdebit_hidden'] as $key => $payreceivable_accountdebit_list ) {
		$taxonomi_code_arr=explode(" - ",$payreceivable_accountdebit_list);
	...
	print_r($create_arr);
	print_r($set_rekening);
	print_r($global->payreceivable->create_payable($create_arr,0,$set_rekening));
	*/
	//create payable payment
	if(!$global->payreceivable->create_payable($create_arr,0,$set_rekening)){
