...
'position_name'=>	$global->users->users_lang['form_label_users_lang']['position_name'],
);
...
while (($data_csv = fgetcsv($handle, 0, ",",'"')) !== FALSE)
	...
	if($i==0)
		...
		$global->db_insert_update('position',$insert_arr,'position_code');
		}
	$i++;
	}
//check if break
...
*/
$file_name="jabatan.csv";
$csv_source="csv/".$file_name;

//default view
$search_value="";
$sort_value="position_code ASC";
$pageset_value=0;
$per_page_value=0;
...
$position_search_list=$global->tbl_search_list("position","*",$def_arr,$search_field_arr,1,$select_num,$qry_str_sort);
...
//get out
$out=$global->csv_generate($fields_arr,$position_search_list);
