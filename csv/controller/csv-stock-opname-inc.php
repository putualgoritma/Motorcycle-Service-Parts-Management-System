...
*/
if(isset($_POST['btn_import']) && $_FILES['file_source']['size'] < 20485)
{
//upload csv
...
while (($data_csv = fgetcsv($handle, 0, ",",'"')) !== FALSE)
	...
	if($i==0)
		...
		$warehouse_stock_details_quantity=$data['product_stock_opname']-$data['product_stock'];
		$insert_arr = array(
		'warehouse_stock_code'=>	$warehouse_stock_code,
		'warehouse_stock_details_quantity'=>	$warehouse_stock_details_quantity,
		'warehouse_stock_details_opname'=>	$data['product_stock_opname'],
...
exit;
}else if(isset($_POST['btn_import']) && $_FILES['file_source']['size'] >= 20485){
$global->error_message("File CSV terlalu besar");
}else{
/**
* Operation csv generate
*/
$file_name="stok-opname.csv";
...
$product_search_list[$i]['product_stock_opname']=$product_search_list[$i]['product_stock'];
if(trim($product_search_list[$i]['product_astra_code'])==""){
	$product_search_list[$i]['product_astra_code']=$product_search_list[$i]['product_code'];
	}
