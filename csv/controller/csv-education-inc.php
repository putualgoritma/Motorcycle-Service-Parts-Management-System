...
'education_name'=>	$global->users->users_lang['form_label_users_lang']['education_name'],
);
...
while (($data_csv = fgetcsv($handle, 0, ",",'"')) !== FALSE)
	...
	if($i==0)
		...
		$global->db_insert_update('education',$insert_arr,'education_code');
		}
	$i++;
	}
//check if break
...
*/
$file_name="pendidikan.csv";
$csv_source="csv/".$file_name;

//default view
$search_value="";
$sort_value="education_code ASC";
$pageset_value=0;
$per_page_value=0;
...
$education_search_list=$global->tbl_search_list("education","*",$def_arr,$search_field_arr,1,$select_num,$qry_str_sort);
...
//get out
$out=$global->csv_generate($fields_arr,$education_search_list);
