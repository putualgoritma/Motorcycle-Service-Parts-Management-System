...
'product_max_stock'=>	$global->product_order->product_order_lang['form_label_product_order_lang']['product_max_stock'],
);
...
while (($data_csv = fgetcsv($handle, 0, ",",'"')) !== FALSE)
	...
	if($i==0)
		...
		'product_max_stock'=>	$data['product_max_stock'],
		'product_fast_moving'=>	1,
		);
		//update product
		$global->db_update("product",$create_arr,"product_code='".$product_code."'");
		}
	$i++;
	}
//check if break
...
*/
$file_name="fast-moving-barang.csv";
