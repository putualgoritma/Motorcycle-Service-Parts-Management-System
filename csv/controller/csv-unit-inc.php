...
'unit_name'=>	$global->product_order->product_order_lang['form_label_product_order_lang']['unit_name'],
);
...
while (($data_csv = fgetcsv($handle, 0, ",",'"')) !== FALSE)
	...
	if($i==0)
		...
		$global->db_insert_update('unit',$insert_arr,'unit_code');
		}
	$i++;
	}
//check if break
...
*/
$file_name="satuan.csv";
$csv_source="csv/".$file_name;

//default view
$search_value="";
$sort_value="unit_code ASC";
$pageset_value=0;
$per_page_value=0;
...
$unit_search_list=$global->tbl_search_list("unit","*",$def_arr,$search_field_arr,1,$select_num,$qry_str_sort);
...
//get out
$out=$global->csv_generate($fields_arr,$unit_search_list);
