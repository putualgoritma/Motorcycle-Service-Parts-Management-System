...
'category_name'=>	$global->product_order->product_order_lang['form_label_product_order_lang']['category_name'],
);
...
while (($data_csv = fgetcsv($handle, 0, ",",'"')) !== FALSE)
	...
	if($i==0)
		...
		$global->db_insert_update('category',$insert_arr,'category_code');
		}
	$i++;
	}
//check if break
...
*/
$file_name="kategori-barang.csv";
$csv_source="csv/".$file_name;

//default view
$search_value="";
$sort_value="category_code ASC";
$pageset_value=0;
$per_page_value=0;
...
'sort'=>$sort_value,
'join_match'=>" AND category_type='0'",
'join_id'=>""
);
$category_search_list=$global->tbl_search_list("category","*",$def_arr,$search_field_arr,1,$select_num,$qry_str_sort);
...
//get out
$out=$global->csv_generate($fields_arr,$category_search_list);
