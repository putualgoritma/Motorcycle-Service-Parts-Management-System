...
*/
if(isset($_POST['btn_import']) && $_FILES['file_source']['size'] < 600485)
{
//upload csv
...
'product_bprice'=>	$global->product_order->product_order_lang['form_label_product_order_lang']['product_bprice'],
...
while (($data_csv = fgetcsv($handle, 0, ",",'"')) !== FALSE)
	...
	if($i==0)
		...
		$product_row=$global->product_order->db_row("product","product_disc,product_id,product_bprice","product_code='".$product_code."'");
		if($product_row['product_id']>0){
			$product_disc=$product_row['product_disc'];
			$product_bprice=$product_row['product_bprice'];
			}else{
			$product_disc=15;
			$product_bprice =$data['product_sprice']-(($product_disc/100)*$data['product_sprice']);
			}
		
		if($data['product_bprice']>0){
			$product_bprice =$data['product_bprice'];
			}
...
exit;
}else if(isset($_POST['btn_import']) && $_FILES['file_source']['size'] >= 600485){
$global->error_message("File CSV terlalu besar");
}else{
/**
* Operation csv generate
*/
$file_name="part-price.csv";
